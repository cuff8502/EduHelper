# 🗓️ EduHelper 업데이트 로그

## 📌 v1.3.8.0 (2025-12-10)
- 반편성 알고리즘 개선
- 반편성 수동 편집 시 사운드 및 이펙트 추가

## 📌 v1.3.7.0 (2025-12-09)
- 반편성 기능 베타테스트 시작
- UI 개선
- 여러 가지 버그 수정

## 📌 v1.3.6.0 (2025-12-08)
- 창체 평어 추가

## 📌 v1.3.5.0 (2025-12-03)
- 음소거 단축키 기능 지원
- 국정 평어 순화
- 여러 가지 버그 수정

## 📌 v1.3.4.0 (2025-12-02)
- 음소거 기능 지원
- 평어에서 국어, 음악 등의 작품/노래 제목 제거
- 평어에서 AI를 ‘인공지능’으로 순화

## 📌 v1.3.3.0 (2025-12-01)
- 메인화면 시각적 피로도 감소
- 창체 자율활동, 동아리 가나다순 정렬 및 카테고리 분류

## 📌 v1.3.2.0 (2025-11-30)
- 홍보문구 복사 기능 추가
- 일부 행발 영어식 표현 순화

---

## 📌 v1.3.1 (2025-11-23)
- 전학년 전교과 평어 업데이트 완료
- 복사 버튼 실행 시 버튼 깜빡임 & 소리 추가
- 사용자의 시각적 피로도를 줄이는 UI 적용
- 설문 링크 추가

---

## 📌 v1.2.9
- 뒤로가기 버튼이 되지 않는 버그 수정
- 국정교과서 단원별 교과평가 업데이트

---

## 📌 v1.2.8 (2025-08-28)
- 단축키 다양화(기존 Ctrl+C 대신 V, Ctrl+V, C 키도 동일 기능 수행)
- 더 이상 별점 3점에 ‘우수한, 뛰어난’ 등의 필요 이상의 긍정 표현을 사용하지 않음

---

## 📌 v1.2.7 (2025-08-18)
- UI 대폭 개선
- 긍정형, 부정형 평어 구분

---

## 📌 v1.2.6 (2025-07-18)
- 프로그램 시작 시 팝업창 추가
- 창체 동아리 활동 세분화 및 평어 다양성 추가
- 창체 동아리 유형 추가

---

## 📌 v1.2.5 (2025-07-09)
- 학생 행동특성 대규모 업데이트
- 행동특성 누가기록 메뉴 신설
- 행동특성 누가기록과 행동특성 및 종합의견 연계
  
---

## 📌 v1.2.3 (2025-06-20)
- UI 개선 및 뒤로가기 버튼이 안먹히는 버그 수정
  
---

## 📌 v1.2.3 (2025-06-05)
- 윈도우 작업표시줄 앱이름 'EduHelper'로 변경
- 자율활동 마우스버튼 버그 수정
- 자율활동 문장 온점 뒤 띄어쓰기 2칸이 되는 버그 수정(띄어쓰기 1칸으로 변경)

---

## 📌 v1.2.1 (2025-06-03)
- 깃허브 공식 소개 저장소(EduHelper) 오픈
- 자율활동 마우스버튼 버그 발견(다음 업데이트시 수정 예정, 당분간 키보드 단축키로 대체 사용 권장)

---

## 📌 v1.2.0 (2025-05-XX)
- 창체기능 추가
- UI 대폭 개선

---

## 📌 v1.1.0 (2025-03-XX)
- 자동 문장 추천 시스템 구현 (⭐ 별점 기반)
- 기타(+) / 기타(-) 버튼 기능 추가
- 문장 중복 방지 기능 탑재
- Copy, Reset, Back 버튼 단축키 연결 (Ctrl+C, ESC 등)

---

## 📌 v1.0.0 (초기 버전)
- 별점 선택 → 평가 문장 자동 출력
- TextEdit → 결과 복사 기능 구현
